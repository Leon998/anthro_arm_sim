...
class DMPs(object):
    ...
    def imitate_path(self, y_des, plot=False):
        ...
        path = np.zeros((self.n_dmps, self.timesteps))  # self.cs.run_time = 1 for discrete system
        x = np.linspace(0, self.cs.run_time, y_des.shape[1])
        # x is a phase parameter that ranges from 0 to 1
        for d in range(self.n_dmps):
            path_gen = scipy.interpolate.interp1d(x, y_des[d])
            for t in range(self.timesteps):
                path[d, t] = path_gen(t * self.dt)
        y_des = path
        print('y_des after interpolate:', y_des.shape)
