...
class CouplingTerm3DDistance:
    """Couples distance between 3D positions.
    ...
    def coupling(self, y, yd=None):
        ...
        C12dot = F12 * self.c2 * self.lf[0]
        C21dot = F21 * self.c2 * self.lf[1]
        return (np.hstack([C12, C21]),
                np.hstack([C12dot, C21dot]))
    

class CouplingTermTriple3DDistance:
    """Couples distance between 3D positions of triple DMPs.

    For :class:`DMP`.

    Parameters
    ----------
    shoulder_position : array-like, shape (3,),
        Shoulder position.

    desired_distance : array-like, shape (3,), float
        Desired distance between components.

    lf : array-like, shape (5,)
        Binary values that indicate which DMP(s) will be adapted.
        The variable lf defines the relation leader-follower. If lf[0] = lf[1],
        then both robots will adapt their trajectories to follow average
        trajectories at the defined distance dd between them [..]. On the other
    ...
    c2 : float, optional (default: 30)
        Scaling factor for spring forces in the acceleration component.
    """
    def __init__(self, shoulder_position, desired_distance, lf, k=1.0, c1=1.0, c2=30.0):
    ...
    def coupling(self, y, yd=None):
        L01_actual_distance = self.shoulder_position - y[:3]
        L01_desired_distance = (np.abs(self.desired_distance[0]) * L01_actual_distance
                            / np.linalg.norm(L01_actual_distance))
        F01 = self.k * (L01_desired_distance - L01_actual_distance)
        F10 = -F01

        L12_actual_distance = y[:3] - y[3:6]
        L12_desired_distance = (np.abs(self.desired_distance[1]) * L12_actual_distance
                            / np.linalg.norm(L12_actual_distance))
        F12 = self.k * (L12_desired_distance - L12_actual_distance)
        F21 = -F12
        
        L23_actual_distance = y[3:6] - y[6:9]
        L23_desired_distance = (np.abs(self.desired_distance[2]) * L23_actual_distance
                            / np.linalg.norm(L23_actual_distance))
        F23 = self.k * (L23_desired_distance - L23_actual_distance)
        F32 = -F23

        C1n = self.c1 * F10 * self.lf[0] + self.c1 * F12 * self.lf[1]
        C2n = self.c1 * F21 * self.lf[2] + self.c1 * F23 * self.lf[3]
        C32 = self.c1 * F32 * self.lf[4]
        C1ndot = F10 * self.c2 * self.lf[0] + F12 * self.c2 * self.lf[1]
        C2ndot = F21 * self.c2 * self.lf[2]  + F23 * self.c2 * self.lf[3]
        C32dot = F32 * self.c2 * self.lf[4]
        return (np.hstack([C1n, C2n, C32]),
                np.hstack([C1ndot, C2ndot, C32dot]))
