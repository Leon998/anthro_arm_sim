...
from movement_primitives.dmp import DMP, CouplingTerm3DDistance
...
## IMITATE
new_Y = np.hstack((new_ts_base2wr, new_ts_base2ee))
dmp.configure(start_y=new_Y[0], goal_y=new_Y[-1])
# compute desired distance
wr_position = new_Y[0][0:3]
ee_position = new_Y[0][3:6]
dd = np.linalg.norm(wr_position - ee_position)
print(dd)
ct = CouplingTerm3DDistance(desired_distance=dd, lf=(1.0, 0.0), k=1.0, c1=30, c2=100.0)
# ct = None

imitate_T, imitate_Y = dmp.open_loop(coupling_term=ct)
imitae_ts_base2wr = imitate_Y[:,0:3]
imitae_ts_base2ee = imitate_Y[:,3:6]
